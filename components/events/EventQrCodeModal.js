...
EventQrCodeModal.propTypes = {
  isOpen: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  event: PropTypes.object.isRequired,
};

function EventQrCodeModal(props) {
  const { event, isOpen, onClose, ...modalProps } = props;

  const [loading, setLoading] = useState(false);

  useDisableBodyScrolling('EventQrCodeModal', isOpen);

  const onSubmit = async values => {
    setLoading(true);

    try {
      const response = await fetch('/api/mails/qr-code', {
        method: 'POST',
    ...
    >
      <div className="container">
        ...
        <div className="content">
          <h1>
            {__('EventQrCodeModal.title', {
              venue: event.organiser.venue.name,
            })}
          </h1>
          <p>{_o(event.tickets.qrCodeInfo)}</p>
          <EmailForm
            onSubmit={onSubmit}
            submitButtonText={__('EventQrCodeModal.getQrCode')}
...
export default memo(EventQrCodeModal);
